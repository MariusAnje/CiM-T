...
parser.add_argument(
    ...
    help="supported dataset including : 1. quantization"
    )
parser.add_argument(
    '-d', '--dataset',
    default='CIFAR10',
...
parser.add_argument(
    ...
    type=int,
    default=5,
    help="the total epochs for quantized model re-training, default is 5"
    )
parser.add_argument(
    '-ep', '--episodes',
    type=int,
...
def quantization_search(device, dir='experiment'):
    ...
    arch_paras = [
        {'filter_height': 3, 'filter_width': 3,
         ...
         'num_filters':  32, 'pool_size': 1},
        {'filter_height': 3, 'filter_width': 3,
         'stride_height': 1, 'stride_width': 1,
         'num_filters':  32, 'pool_size': 2},
        {'filter_height': 3, 'filter_width': 3,
         'stride_height': 1, 'stride_width': 1,
        ...
        {'filter_height': 3, 'filter_width': 3,
         'stride_height': 1, 'stride_width': 1,
         'num_filters': 128, 'pool_size': 1},
        {'filter_height': 3, 'filter_width': 3,
         'stride_height': 1, 'stride_width': 1,
         'num_filters': 128, 'pool_size': 2}]
    ...
    for e in range(args.episodes):
        ...
        quant_flag = True
        for item in quan_paras:
            act_bits = item['act_num_int_bits'] + item['act_num_frac_bits']
            weight_bits = item['weight_num_int_bits'] + item['weight_num_frac_bits']
            print(act_bits, weight_bits)
            if act_bits > 8 or weight_bits > 4:
                quant_flag = False
                break
        
        if fpga_model.validate() and quant_flag:
            quant_model, optimizer = child.get_model(
                    input_shape, arch_paras, num_classes, device,
                    multi_gpu=args.multi_gpu, do_bn=False)
            quant_model.load_state_dict(saved_state_dict)
            _, reward = backend.fit(
                quant_model, optimizer, train_data = train_data, val_data=val_data,
                epochs=args.epochs2, verbosity=args.verbosity)
        else:
            reward = 0
